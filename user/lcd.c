...
void lcd_task()
{
		...
		if(f_LCD==1)
	{
		...
		if(jiemian_f==0)
		{
					...
					if(fre_A<1000 && fre_A>0)
					sprintf(text,"     A=%dHz     ",fre_A);
					else if(fre_A>=1000)
					{
						float f_a=fre_A/1000.0;
						sprintf(text,"     A=%.2fKHz      ",f_a);
					}
					else
						sprintf(text,"     A=NULL      ");
					LCD_DisplayStringLine(Line4,(u8*)text);
					if(fre_B<1000 && fre_B>0)
					sprintf(text,"     B=%dHz       ",fre_B);
					else if(fre_B>=1000)
					{
						float f_B=fre_B/1000.0;
						sprintf(text,"     B=%.2fKHz      ",f_B);
					}
					else
						sprintf(text,"     B=NULL      ");
		...
		{			float f_a=1.0/fre_A;  //周期
					float f_b=1.0/fre_B;
					sprintf(text,"        DATA     ");
					LCD_DisplayStringLine(Line2,(u8*)text);
					if(f_a<0.001 && f_a>0)   //小于1ms
					sprintf(text,"     A=%duS     ",(int)(f_a*1000000));
					else if(f_a>=0.001)
					{
						
						sprintf(text,"     A=%.2fmS      ",(f_a*1000));
					}
					else
						sprintf(text,"     A=NULL      ");
					LCD_DisplayStringLine(Line4,(u8*)text);
					if(f_b<0.001 && f_b>0)
					sprintf(text,"     B=%duS       ",(int)(f_b*1000000));
					else if(f_b>=0.001)
					{
						sprintf(text,"     B=%.2fmS      ",f_b*1000);
					}
					else
						sprintf(text,"     B=NULL      ");
					LCD_DisplayStringLine(Line5,(u8*)text);
